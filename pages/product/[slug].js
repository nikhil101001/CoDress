...
const Post = ({ addToCart, product, variants, buyNow }) => {
  ...
  const checkServiceability = async () => {
    ...
    if (Object.keys(pinJson).includes(pin)) {
      ...
      toast.success("yeah, Pincode is Servicable", {
  ...
  const refreshVariant = (newSize, newColor) => {
    console.log("variant is ", variants, newSize, newColor);
      ...
      <section className="body-font overflow-hidden text-gray-600">
        ...
        <div className="container px-5 py-12 mx-auto">
          <div className="lg:w-4/5 mx-auto flex flex-wrap">
            <img
              alt="ecommerce"
              className="lg:w-1/2 w-full px-12 object-cover object-center rounded "
              src={product.img}
            />
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                    ...
                    Object.keys(variants["white"]).includes(size) && (
                      <button
                        ...
                        className={`border-2 rounded-full w-6 h-6 focus:outline-none ${
                          color === "white"
                            ? "border-black shadow-gray-300 shadow-inner"
                            : "border-gray-300"
                        }`}
                      ></button>
                    )}
                ...
                <div className="flex ml-6 items-center">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="rounded appearance-none shadow-gray-400 shadow-inner py-2 focus:border-black text-base pl-3 pr-10"
                    >
              ...
              <div className="flex">
                ...
                <button
                  // setting the cart and its value
                  onClick={() => {
                    addToCart(
                      slug,
                      1,
                  ...
                  className="flex ml-auto text-white bg-black border-0 py-2 text-sm md:text-base px-4 focus:outline-none hover:bg-slate-800 rounded"
                >
                  Add to cart
                </button>
                <Link href="/checkout">
                  <button
                    ...
                    className="flex ml-2 text-white bg-black border-0 py-2 text-sm md:text-base px-4 focus:outline-none hover:bg-slate-800 rounded"
                  >
                    Buy Now
                  </button>
                </Link>
              </div>
              <div className="pin flex space-x-3  mt-4 text-sm justify-center md:justify-start">
                <input
                  type="text"
                  name="pibcode"
                  id="pincode"
                  className="border-black border-2 px-2 rounded"
                  placeholder="Enter your Pincode"
                  onChange={onChangePin}
                />
                <button
                  onClick={checkServiceability}
                  className="flex text-white bg-black border-0 py-2 px-4 focus:outline-none hover:bg-slate-800 rounded"
                >
                  Check
                </button>
              </div>
              {service && (
                <div className="text-green-600 text-sm my-2">
                  Yup! Service is available for this region
                </div>
              )}
              {!service && service != null && (
                <div className="text-red-600 text-sm my-2">
                  Sorry! Service is not available for this region yet
                </div>
              )}
            </div>
          </div>
...
export async function getServerSideProps(context) {
  ...
  let colorSizeSlug = {}; // {red: {Xl: {slug: oversized-tshirt-xl}}}
